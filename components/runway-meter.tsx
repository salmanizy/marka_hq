...
type Status = "good" | "warning" | "danger" | "neutral" | "profitable"

function getStatus(months: number | null, isProfitable: boolean): Status {
  if (isProfitable) return "profitable"
...
const STATUS_CONFIG: Record<Status, {
  ...
  profitable: { border: "border-l-emerald-500", bar: "bg-emerald-500", badge: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-800", label: "Profitable — burn negatif ✓" },
...
export function RunwayMeter({ months, totalCash, monthlyNetBurn, isProfitable }: RunwayMeterProps) {
  const status = getStatus(months, isProfitable)
  const cfg = STATUS_CONFIG[status]
  const gaugePercent = months !== null ? Math.min((months / MAX_MONTHS) * 100, 100) : isProfitable ? 100 : 0
    ...
    <Card className={cn("shadow-sm border-l-4", cfg.border)}>
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="top" className="max-w-[240px] text-xs">
                Current Cash Balance ÷ Monthly Net Burn Rate. Net Burn = Monthly Expense − Monthly Revenue
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      ...
      <CardContent className="px-5 pb-5">
        ...
        <div className="flex items-baseline gap-2 mb-4">
          {isProfitable ? (
            <span className="text-3xl font-semibold tracking-tight text-emerald-600 dark:text-emerald-400">∞</span>
          ) : (
            <>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Current Cash Balance</p>
            <p className="text-sm font-medium">{formatCurrency(totalCash)}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Monthly Net Burn</p>
            <p className={cn("text-sm font-medium", isProfitable && "text-emerald-600 dark:text-emerald-400")}>
              {isProfitable ? "−" : ""}{formatCurrency(Math.abs(monthlyNetBurn))}
            </p>
          </div>
        </div>
      </CardContent>
